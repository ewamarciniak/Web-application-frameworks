class FoodIntakeLog < ActiveRecord::Base
  ...
  def calculate_calories_intake
    return self.food ? self.food.calories / 100.0 * self.amount : nil
  end

  def calculate_carbohydrates_intake
    return self.food ? self.food.carbohydrates / 100.0 * self.amount  : nil
  end

  def calculate_cholesterol_intake
    return self.food ? self.food.cholesterol / 100.0 * self.amount : nil
  end

  def calculate_fibre_intake
    return self.food ? self.food.fibre / 100.0 * self.amount : nil
  end

  def calculate_protein_intake
    return self.food ? self.food.protein / 100.0 * self.amount : nil
  end

  def calculate_saturated_fats_intake
    return self.food ? self.food.saturated_fats / 100.0 * self.amount : nil
  end

  def calculate_unsaturated_fats_intake
    return self.food ? self.food.unsaturated_fats / 100.0 * self.amount : nil
  end

  def calculate_sugars_intake
    return self.food ? self.food.sugars / 100.0 * self.amount : nil
  end
end